...
def interact_with_user(exercise, num_sets, warmup=False): #dont need the warmup parameter it seems
    ...
    if num_sets == 1:
        ...
        for (percent, reps, setNumber) in [(.4, int(inp2), 1), (.7, int(0.7 * int(inp2)), 2), (1, int(0.5 * int(inp2)), 3)]:
            warmup_routine(reps, percent, setNumber)
    elif num_sets == 4:
        for (percent, reps, setNumber) in [(.4, int(inp2), 1), (.6, int(0.7 * int(inp2)), 2), (.8, int(0.5 * int(inp2)), 3), (1, int(0.2 * int(inp2)), 4)]:
            warmup_routine(reps, percent, setNumber)
    elif num_sets == 5:
        for (percent, reps, setNumber) in [(.3, int(inp2), 1), (.5, int(0.8 * int(inp2)), 2), (.7,  int(0.6 * int(inp2)), 3), (1, int(0.4 * int(inp2)), 4), (.9, int(0.2 * int(inp2)), 5)]:
            warmup_routine(reps, percent, setNumber)
    else:
...
def main():
    ...
    if todays_routine:
        ...
        print("Today is " + current_day + " and you will be working on " + exercisesInMuscleGroup + "so have a great workout!")
        for muscle_group in todays_routine:
